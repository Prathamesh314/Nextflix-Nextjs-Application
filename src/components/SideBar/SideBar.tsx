...
function SideBar() {
  ...
  return (
    <div className="p-2 flex m-0 w-screen">
      <div>
        <div>
          <h2 className='pl-10 pr-4 mb-2 italic text-[22px] font-semibold cursor-pointer underline decoration-wavy text-white'>Categories</h2>
        </div>
        ...
        <div className='grid grid-row-20 grid-flow-row mt-4'>
          {categories.map((item, index) => (
            <button key={index} className={`row-span-1 font-sans h-10 w-50 text-lg rounded-xl bg-purple-950 mb-2 hover:bg-red-700 hover:scale-75 transition-all ${item == genre ? 'border-yellow-400 border-[2px] bg-red-600' : 'bg-purple-950'}`}
