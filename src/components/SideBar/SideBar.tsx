...
function SideBar() {
  ...
  return (
    <div className="p-2 flex m-0 w-screen">
      <div>
        <div>
          <h2 className='pl-10 pr-4 mb-2 italic text-[22px] font-semibold cursor-pointer underline decoration-wavy text-yellow-400'>Categories</h2>
        </div>
        <div className='grid grid-row-20 grid-flow-row mt-4'>
          {types.map((item, index) => (
            <button key={index} className={`row-span-1 h-10 w-50 text-lg rounded-xl mb-2 hover:bg-green-700 hover:scale-75 transition-all ${item == selectedIndex ? 'border-red-400 border-[2px] bg-purple-700' : 'bg-purple-950'}`}
              onClick={() => setSlectedIndex(item)}
            >
              <h2 className='font-sans'>{item}</h2>
            </button>
          ))}
        </div>
        ...
        <div className='grid grid-row-20 grid-flow-row mt-4'>
          {categories.map((item, index) => (
            <button key={index} className={`row-span-1 font-sans h-10 w-50 text-lg rounded-xl  mb-2 hover:bg-red-700 hover:scale-75 transition-all ${item == genre ? 'border-red-400 border-[2px] bg-purple-700' : 'bg-purple-950'}`}
